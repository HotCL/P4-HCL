...
This section will describe HCL designed for this project.
It will also explain the functionality and features.

During the problem analysis, it was decided that HCL, would be designed to be used as a preliminary introduction to programming.
HCL should reflect this decision, by being useful, approachable and easy to understand for users that might not know programming and programming fundamentals.

It was decided that the syntax of HCL should reflect the English language to some extend.
...
In conjunction to designing according to the English language and the interest in utilizing high-order functions, HCL's design also draws inspiration from the functional programming paradigm.
In an effort to accommodate these ideas, it was decided that almost all operations in HCL would be done using functions. 

Since HCL is supposed to be used in correlation with the Arduino platform, it is necessary to take the Arduino's \textbf{\textit{setup}} and \textbf{\textit{loop}} functions into account, which are explained in section \ref{arduinoC}.
...
The following is a rundown of the features and conventions of HCL's syntax, that is planned for implementation:
...
This means that all arithmetic operators are infix-functions that take two numbers as parameters. 
The only operators that remain operators are the assign-operator (\textbf{=}) and the encapsulation-operators, namely parenthesis, square-brackets and curly-brackets.

Not having operators for arithmetic operations also means that there are no native operator precedence in HCL.
This can be amended by structuring calculations in a left-to-right manner, or by encapsulating operations with parentheses.
...
This makes sense when using lambdas, where return statements are often implicit. 
...
Function calls are done with infix notation, meaning that if a given method is defined with parameters, the first argument has to appear before the function identifier, while all remaining arguments must appear after, in sequence.
...
As mentioned in section \ref{subsec:hofhcl}, HCL will include high-order functionality. 
...
\textbf{Overloading}\\
HCL will include function overloading. 
...
Since the ability to overload with different amount of parameters brings next to nothing to HCL, it was decided that it was not worth the effort to implement.
...
\subsection{Generics}
HCL will include simple generics.
