...
There are three different ways to call a function depending on how many parameters must be passed with the function:
\begin{itemize}
	\item If no parameters must be passed, then invoking the function identifier is a call in and of itself.
	\item If one parameter must be passed, then the parameter must appear on the left-hand side of the function identifier.
	\item If more than one parameter must be passed, the first parameter must appear on the left-hand side of the function identifier, and the rest must appear, in order, on the right-hand side of the function.
\end{itemize}
