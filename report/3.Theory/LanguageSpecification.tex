...
Since the language is supposed to be used in correlation with the Arduino platform, it is nessecary to take the Arduino's \textbf{\textit{setup}} and \textbf{\textit{loop}} functions into account, which are explained in section \ref{arduinoC}.
...
\textbf{Types}\\
The language includes only three primitive types, namely numbers (\textbf{\textit{num}}), text-strings (\textbf{\textit{txt}}) and the boolean type (\textbf{\textit{bool}}).\\
...
Dictionaries are collections of key-value pairs, are deemed NTH and are not planned for implementation. 
...
Also included is the type \textbf{\textit{none}} which in some languages, particularly C\# and java, is known as the \textbf{\textit{void}} keyword. 

\textbf{Type Declaration}\\
Types can be declared in a few different ways. The teacher from the preliminary interviews, mentioned that sometimes, he wanted to teach types, while other times, it didn't matter.
...
The type of a methods parameters, as well as its return type, are explicitly declared when a function is defined. 
...
The parameters of the method is declared on the right-hand-side inside parentheses, followed by an arrow (\textbf{\textit{->}}), which declares the return type. 
...
When functions are passed in as parameters, they will be prefixed with '::' characters to differentiate between functions as parameters and other function calls. 
...
Loops are created using either of the predefined methods \textbf{\textit{foreach}} or \textbf{\textit{do}} that takes a \textbf{\textit{list}} as its first parameter and \textbf{\textit{func}} as its second parameter.
...
\end{lstlisting}
There are no \textbf{\textit{while}} actual loop structures in the language, and all looping will actually be done with iterations.
