...
To do this it is necessary to construct a PDA.
The reason being that the set of context free languages is equivalent to the set of languages recognized by push down automata.
...
The final NPDA can be seen in Appendix 2 on page \pageref{NPDA}.
...
if that is the case, it is possible to create a FSA that recognizes the language. 
Since the set of regular languages is a subset of the context free languages, this might be the case.
An FSA is more trivial to implement than a NPDA. 
...
Since HCL does not uphold the required property for it to be a regular language, it is not, and the only automaton capable of recognizing the language is the NDPA described in the previous sections.

\subsection{Types and Identifiers}
So far the syntax of HCL has been properly defined as a CFL. 
Meaning that all variables, terminals and the transition rules for recognizing HCL has been described in a computational model as seen in appendix 2 on page \pageref{NPDA}.
...
In conclusion HCL is a context free language, meaning that it can be described by a NPDA.
