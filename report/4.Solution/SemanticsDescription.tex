...
$eOrL ::= e | L$\\
$F_{call} ::= f | eOrL\ f\ eOrL^*$\\
$F_{exp} ::= F_{call} |  pOrf\ =\ ([T\ x]\sim):\ T\ L;\ S |T\ pOrf;\ S$\\
$L ::= \{\ S\ \} | \{\ S\ return\ e\ \} | \{\ S\ return\ S\ \} | \{\ S;\ e\ \}$\\
$S ::= x = e | S_1;S_2 | b\ then\ L | L\ while\ b | list\ forEach\ L | p | eOrL\ p\ eOrL^* | F_{exp} | D_V | D_P | \epsilon$\\
$D_v ::= T\ x\ =\ e;\ S | T\ x\ = call_{exp};\ S | T\ x;\ D_v | \epsilon$\\
$D_p ::= T\ pOrf\ =\ ([T\ x]\sim):\ T\ L;\ S | \epsilon$
