...
An expression is any syntactic component that returns or has a value.
In HCL most expressions behave the same, but a few transition-systems uses special notation.
...
With the the given variable environment $env_V$ and procedure environment $env_P$ the execution of the statement $S$ will change the store $sto$ to $sto'$ and the expression $e$ will be evaluated to $v$.
The transition-system for $L$ is ($Lambda\ X\ Env_V\ X\ Env_P, \rightarrow_L, sto \cup Lit$).
...
\begin{center}
	...
	\cfrac
	{env_V, env_P \vdash \langle S, sto \rangle \rightarrow_L sto', v}
	{env_V, env_P \vdash \langle \{\ S\ e\ \}, sto \rangle \rightarrow_L sto', v}
	\end{math}
...
In conclusion, HCL is best described as a functional language with side effects. 
